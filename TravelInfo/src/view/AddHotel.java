...
public class AddHotel extends JPanel implements ActionListener {
	...
	// 声明标签数组
	private JLabel[] jlArray = new JLabel[] { new JLabel("酒店名称："), new JLabel("酒店位置："), new JLabel("酒店价格："),
			new JLabel("房间总数："), new JLabel("剩余数量：") };
		...
		if (e.getSource() == btn) {
			...
			for (int i = 0; i < jtxtArray.length; i++) {
				if (jtxtArray[i].getText().trim().equals("")) {
					// 当各文本框为空提示
					JOptionPane.showMessageDialog(this, "酒店信息不能为空！", "提示消息", JOptionPane.WARNING_MESSAGE);
					return;
				}
				str[i] = jtxtArray[i].getText().trim();
			}
			if (!validateNum(str[4], str[2], str[3])) {
				JOptionPane.showMessageDialog(this, "酒店信息添加失败！\n请检查所填信息类型是否正确！", "提示消息", JOptionPane.WARNING_MESSAGE);
				return;
			}
			...
			if (flag) {
				...
				JOptionPane.showMessageDialog(this, "酒店信息添加成功！", "提示消息", JOptionPane.INFORMATION_MESSAGE);
				return;
			}
